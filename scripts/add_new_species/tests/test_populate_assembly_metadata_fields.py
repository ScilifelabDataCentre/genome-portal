...
def test_populate_config_yml_with_assembly_metadata(
    assembly_metadata: AssemblyMetadata, user_data_tracks: list[dict], tmp_path: Path
):
    """
    ...
    and user_data_tracks by process_data_tracks_Excel().
    """

    populate_config_yml(assembly_metadata, user_data_tracks, tmp_path)
    ...
    assert len(updated_config["tracks"]) == len(user_data_tracks) - 1
