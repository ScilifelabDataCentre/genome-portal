...
const handleButtonClick = (button, codeBlock, clipboard) => {
    clipboard.writeText(codeBlock.innerText).then(
        () => {
            button.blur();
            button.innerHTML = '';
            button.appendChild(document.createTextNode(" Copied "));
            button.appendChild(createSvgElement(svgCheck));
            setTimeout(() => {
                button.innerHTML = '';
                button.appendChild(document.createTextNode(" Copy "));
                button.appendChild(createSvgElement(svgCopy));
            }, 2000);
        },
        (error) => {
...
const createCopyButton = (codeBlock, clipboard) => {
    const button = document.createElement("button");
    button.className = "clipboard-button";
    button.type = "button";
    button.appendChild(document.createTextNode(" Copy "));
    button.appendChild(createSvgElement(svgCopy));
    button.addEventListener("click", () => handleButtonClick(button, codeBlock, clipboard));
    return button;
};

const addCopyButtons = (clipboard) => {
    document.querySelectorAll("pre > code").forEach((codeBlock) => {
        const button = createCopyButton(codeBlock, clipboard);
        const clipboardDiv = document.createElement("div");
        clipboardDiv.className = "clipboard";
