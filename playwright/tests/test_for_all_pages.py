...
def test_for_placeholders(all_pages: list[Page]):
    """
    Check for any TODOs or "TODOs-like" comments in the pages.
    """
    PLACEHOLDERS = ["TODO", "FIXME", "XXXXXX"]
    for page in all_pages:
        for placeholder in PLACEHOLDERS:
            locator = page.get_by_text(placeholder)
            expect(
                locator,
                f"Found the word '{placeholder}' on this page: {page.url}",
            ).to_have_count(0)
