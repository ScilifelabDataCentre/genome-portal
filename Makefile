...
TRIX_FILES := $(foreach dir,$(patsubst $(CONFIG_DIR)/%, $(DATA_DIR)/%, $(SPECIES_DIRS)), $(wildcard $(dir)/trix/*))
TRIX_DIRS := $(foreach dir,$(patsubst $(CONFIG_DIR)/%, $(DATA_DIR)/%, $(SPECIES_DIRS)), $(wildcard $(dir)/trix))
...
.PHONY: text-index
text-index:
	@for dir in $(sort $(patsubst $(CONFIG_DIR)/%, $(DATA_DIR)/%, $(SPECIES_DIRS))); do \
		if [ -d "$$dir" ]; then \
			mkdir -p "$$dir/trix"; \
